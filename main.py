...
# TamilSentiMix dataseti kullanılmıştır.

# Veri setinin yüklenmesi
def loadCsv(filename):
    lines = csv.reader(open('Tamil_first_ready_for_sentiment.csv'))
    dataset = list(lines)
...
def main():
    filename = 'Tamil_first_ready_for_sentiment.csv'
    splitRatio = 0.67
    dataset = loadCsv(filename)
    # print(dataset)
    trainingSet, testSet = splitDataset(dataset, splitRatio)
    print('{0} Satırın içinden {1} Eğitim verisi ve {2} Test verisi kullanılmıştır.'.format(len(dataset), len(trainingSet), len(testSet)))
    # prepare model
    summaries = summerizeByClass(trainingSet)
    # test model
    predictions = getPredictions(summaries, testSet)
    accuracy = getAccuracy(testSet, predictions)
    print('Doğruluk Oranı(Yüzde)= %{0}'.format(accuracy))
